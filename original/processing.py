...
async def main():
    ...
    USE_SUBSET = parse_bool(
        config["SYSTEM"]["USE_SUBSET"]
    ...
    )  # Set to the number of chunks you want to use if you're using a subset. If you're not using a subset, this will be ignored.

    USE_FILENAMES = parse_bool(
        config["SYSTEM"]["USE_FILENAMES"]
    ...
    SKIP_CONVERSATION_GENERATION = parse_bool(
        config["SKIP"]["CONVERSATION_GENERATION"]
    )  # useful if you're generating "tight" data only.

    if USE_GUTENBERG:
        print(
            "SCRAPING IS ON. BEGINNING GUTENBERG SCRAPE! This will modify your input folder."
        )
        steps.scrape_text_using_config(
            start_url=START_URL, max_books=MAX_BOOKS, max_failures=MAX_FAILURES
        )
    ...
    sentence_chunks = []
    content_list = []
    for source_text in tqdm(
        source_texts, desc="Reading, OCR-ing, and Chunking Input Files..."
    ):
        chunks, content = (
            augmentoolkit.utils.sentence_chunking_algorithm.sentence_chunking_algorithm(
    ...
    try:
        ...
        print(
            "No paragraphs processed. Likely you have the wrong input directory path, or there's nothing in there. Check your input directory path?"
        )
        sys.exit(1)
    ...
    if (
        "localhost"
        or "127.0.0." in LARGE_BASE_URL
        or "localhost"
        or "127.0.0." in SMALL_BASE_URL
    ):
        print(
    ...
    if SKIP_FILTER_CHUNKS:
        ...
        steps.convert_logging_to_dataset(
            input_pth=os.path.join(
                "judge_paragraph_generations", "intermediate_generations"
            ),
            output_pth="judge_paragraph_generations",
        )
    ...
    for idx, future in tqdm(sorted(limited_tasks_qgen, key=lambda x: x[0])):
        await future
    ...
    if WORK_IN_PHASES and PHASE_INDEX == 1:
        print(
            "EXITING DUE TO config.yaml SETTINGS AROUND PHASES; SET TO ONLY EXECUTE PHASE 1 RIGHT NOW"
        )
        sys.exit(0)
    ####
    ...
    if WORK_IN_PHASES and PHASE_INDEX == 2:
        print(
            "EXITING DUE TO config.yaml SETTINGS AROUND PHASES; SET TO ONLY EXECUTE PHASE 2 RIGHT NOW"
        )
        sys.exit(0)
    ...
    vetted_qa_dicts = [
        qadict
        for qadict in vetted_qa_dicts
        if filter_the_text(qadict["question"]) and filter_the_text(qadict["answer"])
    ]
    ...
    steps.convert_revised_questions_to_question_generation_training(
        qa_dicts_by_text=qa_dicts_by_text, use_filenames=USE_FILENAMES
    )
    if SKIP_CONVERSATION_GENERATION:
        ...
        for idx, future in tqdm(sorted(limited_tasks_convwriting, key=lambda x: x[0])):
            await future
    ...
    if USE_SUBSET:
        print(
            f"Warning! USE_SUBSET was on in the config you used, {config_path}. This means that you only generated data from the first {SUBSET_SIZE} chunks of your input data. If you want to generate data from all chunks, set USE_SUBSET to False."
        )
